// WARNING: Auto-generated by a compiler. Do not modify!!
 ...
 *      EmscriptenModule
 */
export async function qtLoad(config) {
  ...
  if (typeof config !== "object") throw new Error("config is required, expected an object");
  if (typeof config.qt !== "object") throw new Error("config.qt is required, expected an object");
  ...
  const qtPreRun = (instance) => {
    ...
    for (const [name, value] of Object.entries(config.qt.environment ?? {})) instance.ENV[name] = value;
    ...
    if (!isFsExported) throw new Error("FS must be exported if preload is used");
  ...
  config.locateFile = (filename) => {
    ...
    if (originalLocatedFilename.startsWith("libQt6")) return `${config.qt.qtdir}/lib/${originalLocatedFilename}`;
  ...
  let instance;
  try {
    instance = await Promise.race([circuitBreaker, config.qt.entryFunction(config)]);
    ...
    if (!originalNoInitialRun) instance.callMain(originalArguments);
  } catch (e) {
...
export function QtLoader(qtConfig) {
  const warning =
    "Warning: The QtLoader API is deprecated and will be removed in " +
